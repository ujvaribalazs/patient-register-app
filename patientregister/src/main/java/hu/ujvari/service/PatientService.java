...
public class PatientService {
    ...
    public PatientService(LdapConnector ldapConnector, PatientDbConnector patientDbConnector, AuthService authService, EkgDbConnector ekgDbConnector) {
    ...
    public Patient getPatientByBetegId(String betegId) throws SecurityException {
        ...
        if (!ldapConnector.hasAccessToPatient(currentUser, patient.getPatientId())) {
    ...
    public EkgData getLatestEkgForPatient(String patientId) {
        ...
        if (ekgList != null && !ekgList.isEmpty()) {
            // Esetleg idő szerint rendeznéd – de ha az adatbázisban már így van, akkor csak az első:
            return ekgList.get(0);  // vagy utolsó, ha úgy tárolódik
        }
        return null;
    }
    ...
    public Examination getExaminationById(String examinationId) throws SecurityException {
        ...
        if (!ldapConnector.hasAccessToPatient(currentUser, examination.getPatientId())) {
    ...
    public void saveExamination(Examination examination) throws SecurityException {
        ...
        if (examination.getId() != null && !examination.getId().isEmpty()) {
            ...
            if (existingExamination != null && 
                !ldapConnector.hasAccessToPatient(currentUser, existingExamination.getPatientId())) {
                throw new SecurityException("Nincs jogosultsága ehhez a vizsgálathoz!");
            }
